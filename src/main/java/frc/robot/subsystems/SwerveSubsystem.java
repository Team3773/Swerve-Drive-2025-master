...
public class SwerveSubsystem extends SubsystemBase {
    ...
    public void configureHolonomic() {
        AutoBuilder.configureHolonomic(
                ...
                () -> {
                    // I figure if one of these works it's fine, it only runs once so redundancy is
                    // fine
                    if (DriveTab.getIsPathFlipped() == 1) { // expicit path flip, if this is not set, it is fine because
                                                            // it gets driverstation
    ...
    public void periodic() {
        ...
        // if (visionPose[0] != 0 && visionPose[1] != 0 && m_limelight.getTargetedArea()
        // > VisionConstants.AprilTagMinimumArea &&
        // getDistance(new Pose2d(visionPose[0], visionPose[1], getRotation2d()),
        // PoseEstimator.getEstimatedPosition()) < 2.0){
        ...
        PosePublisher.set(new Pose2d[]{
            // new Pose2d(m_limelight.getBotPose()[0], m_limelight.getBotPose()[1], m_odometry.getPoseMeters().getRotation()), //Vision Pose
    ...
    public void updatePoseEstimater() {
        ...
        // m_photonVision.getVisionPoseEstimationResult();
     ...
     * @param degree where the curve starts to be exponential, 9 is middle 4.5 is
     *               more linear
     */
    public static double herraFCurve(double I, double s, double degree) {
        double funcInput = Math.abs(I);
        double output = I - (Math.copySign(
                Math.pow(funcInput, (s / 9)) * Math.pow((1 - Math.cos(funcInput * Math.PI)) / 2, (9 - s) / degree), I))
                + I;
        if (Math.abs(I) > .02) {
            return output;
        } else {
